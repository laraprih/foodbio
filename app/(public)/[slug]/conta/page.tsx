...
import {
  ...
  ChefHat, CheckCircle, AlertCircle, MessageCircle, BadgeCheck, Send,
} from 'lucide-react'
...
export default function ContaPage() {
  ...
  const [tab, setTab] = useState<'pedidos' | 'dados' | 'enderecos' | 'whatsapp'>('pedidos')
...
function WhatsAppTab({ profile }: { profile: any }) {
  const defaultPhone = profile?.user?.phone ?? ''
  ...
  React.useEffect(() => {
    ...
    fetch(`/api/store/customer/whatsapp/confirm?phone=${normalized}`)
      .then(r => r.json())
      .then(d => { if (d.verified) setStep('verified') })
      .catch(() => {})
  }, [defaultPhone])

  async function handleSend() {
    setError('')
    if (!phone.trim()) { setError('Digite seu número de WhatsApp'); return }
    setSending(true)
    try {
      const r = await fetch('/api/store/customer/whatsapp/request', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function handleConfirm() {
    ...
    setConfirming(true)
    try {
      const r = await fetch('/api/store/customer/whatsapp/confirm', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ phone, code }),
      })
      const d = await r.json()
      if (!r.ok) { setError(d.error ?? 'Código inválido'); return }
      setStep('verified')
      setError('')
    } finally {
      setConfirming(false)
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="px-5 py-4 border-b border-gray-50 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">Receba atualizações dos pedidos</p>
          </div>
          {step === 'verified' && (
        ...
        <div className="p-5 space-y-4">
          {step === 'verified' ? (
            <div className="flex flex-col items-center text-center py-4 gap-3">
              ...
              <div>
                <p className="font-black text-gray-900">WhatsApp verificado!</p>
                <p className="text-sm text-gray-500 mt-1">
                  Você receberá um link de rastreamento no WhatsApp assim que seu pagamento for confirmado.
                </p>
              </div>
              <button
            ...
            <>
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    type="tel"
                    value={phone}
                    onChange={e => { setPhone(e.target.value); setStep('idle'); setError('') }}
                    placeholder="(00) 00000-0000"
                    className="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-green-500/30 focus:border-green-500 transition"
                  />
                  <button
                    onClick={handleSend}
                    disabled={sending}
                    className="px-4 py-2.5 bg-green-500 text-white font-bold text-sm rounded-xl hover:bg-green-600 transition-colors disabled:opacity-50 flex items-center gap-1.5 shrink-0"
                  >
                    {sending ? (
                    ...
                    {step === 'sent' ? 'Reenviar' : 'Enviar código'}
                  </button>
                </div>
              </div>
              ...
              {step === 'sent' && (
                <div>
                  ...
                  <p className="text-xs text-gray-400 mb-2">
                    Digite o código enviado para o WhatsApp <span className="font-semibold text-gray-600">{phone}</span>
                  </p>
                  <div className="flex gap-2">
                    <input
                      type="number"
                      ...
                      placeholder="0000"
                      maxLength={4}
                      className="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 text-sm text-center font-black tracking-[0.5em] focus:outline-none focus:ring-2 focus:ring-green-500/30 focus:border-green-500 transition"
                    />
                    <button
                      onClick={handleConfirm}
                      disabled={confirming || code.length !== 4}
                      className="px-4 py-2.5 bg-[var(--color-lime-primary)] text-white font-bold text-sm rounded-xl hover:brightness-90 transition-all disabled:opacity-50 flex items-center gap-1.5 shrink-0"
                    >
                      {confirming ? (
                        <div className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin" />
                      ) : (
              ...
              {error && (
                <p className={`text-xs font-semibold px-3 py-2 rounded-xl ${
                  error.startsWith('[DEV]') ? 'bg-amber-50 text-amber-700' : 'bg-red-50 text-red-600'
                }`}>
                  {error}
                </p>
              )}
      ...
      <div className="bg-gray-50 rounded-2xl p-4 flex gap-3">
        ...
        <div className="text-xs text-gray-500 leading-relaxed space-y-1">
          ...
          <p>Depois de verificar seu WhatsApp, quando um pedido seu for pago você receberá automaticamente um link para acompanhar o status em tempo real diretamente no WhatsApp.</p>
        </div>
      </div>
    </div>
  )
