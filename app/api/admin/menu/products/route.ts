...
export async function POST(req: NextRequest) {
  ...
  const { name, description, price, categoryId, imageUrl, available, sortOrder } = await req.json()
  ...
  const { rows } = await pool.query(
    `INSERT INTO "Product" (id, "tenantId", "categoryId", name, description, price, "imageUrl", available, "sortOrder")
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
     RETURNING id, "categoryId", name, description, price, "imageUrl", available, "sortOrder"`,
    [id, tenantId, categoryId, name.trim(), description ?? null, Number(price), imageUrl ?? null, available !== false, sortOrder ?? 0]
  )
